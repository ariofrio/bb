...
- App chrome is unselectable by default. Add `select-text` to user-authored, diagnostic, or command content that should support drag selection; add `data-select-all-scope` separately only when Cmd/Ctrl-A should select that content boundary. Cmd/Ctrl-A uses the nearest connected scope from the most recent pointer or focus interaction and suppresses document-wide selection when no scope has been activated. The policy lives in `apps/app/src/app.css`, `apps/app/src/components/AppSelectAllController.tsx`, and `apps/app/src/lib/select-all-scope.ts`, with coverage in `apps/app/src/app-selection.test.ts` and `apps/app/src/components/AppSelectAllController.test.tsx`.
